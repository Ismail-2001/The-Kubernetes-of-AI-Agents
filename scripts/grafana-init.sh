...
ensure_notification_policy() {
  if [ -n "${SLACK_ALERT_WEBHOOK:-}" ]; then
    RECEIVER="E-GAOP Slack"
  else
    RECEIVER="E-GAOP Log"
    echo "Contact point: ${RECEIVER} (no-op webhook sink)"
    SINK_PAYLOAD='{"name":"E-GAOP Log","type":"webhook","settings":{"url":"http://localhost:3000/api/health","sendReminder":false,"autoResolve":true},"disableResolveMessage":true}'
...
ensure_rules() {
  ...
  _upsert_rule \
    ...
    "E-GAOP Service Down" \
    "critical" \
    "One or more E-GAOP services are unreachable" \
    "Service is down" \
    'min(up{job="egaop-services"}) == 0' \
    "30s"

  _upsert_rule \
    "egaop_high_error_rate" \
    "E-GAOP High Error Rate (>5% 5xx)" \
    "critical" \
  ...
  _upsert_rule \
    "egaop_high_latency_p95" \
    "E-GAOP High Latency P95 (>3s)" \
    "warning" \
  ...
  _upsert_rule \
    "egaop_high_latency_p99" \
    "E-GAOP Critical Latency P99 (>10s)" \
    "critical" \
  ...
  _upsert_rule \
    "egaop_collector_dropping" \
    "E-GAOP Metrics Pipeline Dropping" \
    "warning" \
...
_upsert_rule() {
  ...
  PAYLOAD=$(cat <<ENDJSON
{"uid":"${RULE_UID}","title":"${TITLE}","ruleGroup":"egaop-critical","folderUID":"__default__","noDataState":"Alerting","execErrState":"Alerting","for":"${FOR}","annotations":{"summary":"${SUMMARY_ESC}","description":"${DESC_ESC}"},"labels":{"severity":"${SEVERITY}","team":"egaop","rule_type":"grafana_alert"},"data":[{"refId":"A","queryType":"","relativeTimeRange":{"from":0,"to":600},"datasourceUid":"prometheus","model":{"expr":"${EXPR_ESC}","intervalMs":10000,"maxDataPoints":100,"refId":"A"}}],"condition":"A"}
ENDJSON
)
